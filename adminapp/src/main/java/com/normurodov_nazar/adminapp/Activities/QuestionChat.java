...
import com.normurodov_nazar.adminapp.MyD.QuestionChatAdapter;
    ...
    private void showMessages(ArrayList<Message> messages) {
        ...
        adapter = new QuestionChatAdapter(this, messages, questionId,
                ...
                user -> {
                    ...
                    Hey.showPopupMenu(this, bar, new ArrayList<>(Arrays.asList(getString(R.string.profileInfo), getString(R.string.block))), (position, name) -> {
                        if (position == 0) {
                            ...
                            Hey.showDatePicker(this, blockTime -> {
                                LoadingDialog x = Hey.showLoadingDialog(this);
                                DocumentReference userDoc = FirebaseFirestore.getInstance().collection(Keys.users).document(String.valueOf(user.getId()));
                                Hey.isDocumentExists(this, userDoc, new Exists() {
                                    @Override
                                    public void exists(DocumentSnapshot doc) {
                                        Hey.updateDocument(QuestionChat.this, userDoc, Collections.singletonMap(Keys.blockTime, blockTime), doc13 -> {
                                            x.closeDialog();
                                            Hey.showAlertDialog(QuestionChat.this, "BLOCKED UNTIL " + Hey.getTimeText(QuestionChat.this, (long) blockTime));
                                        }, errorMessage -> {
                                            Hey.showAlertDialog(QuestionChat.this, "NOT EXISTS");
                                            x.closeDialog();
                                        });
                                    }
                ...
                (message, itemView, position) -> {
                    ...
                    if (message.getSender() == My.id) {
                                ...
                                d.setOnDismissListener(dialog -> {
                                    if (d.getResult()) {
                                        Hey.amIOnline(new StatusListener() {
                                            @Override
                                            public void online() {
                                                LoadingDialog z = Hey.showLoadingDialog(QuestionChat.this);
                                                Hey.showDeleteImageDialog(QuestionChat.this, message, doc -> Hey.deleteDocument(QuestionChat.this, chats.document(message.getId()), doc1 -> z.closeDialog()), errorMessage -> z.closeDialog());
                                            }
                        ...
                        MyDialogWithTwoButtons x = Hey.showDeleteDialog(this, "DELETE THIS IMAGE", null, false);
                        x.setOnDismissListener(c -> {
                            if (x.getResult()) {
                                Hey.amIOnline(new StatusListener() {
                                    @Override
                                    public void online() {
                                        LoadingDialog a = Hey.showLoadingDialog(QuestionChat.this);
                                        Hey.showDeleteImageDialog(QuestionChat.this, message, doc -> Hey.deleteDocument(QuestionChat.this, chats.document(message.getId()), doc1 -> {
                                            Hey.showToast(QuestionChat.this, "DELETED");
                                            a.closeDialog();
                                        }), errorMessage -> a.closeDialog());
                                    }
