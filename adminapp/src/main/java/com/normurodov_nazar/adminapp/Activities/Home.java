...
import com.normurodov_nazar.adminapp.MyD.HomeFragmentStateAdapter;
        ...
        if (!Hey.amIBlocked()) {
            if (viewPager2.getCurrentItem() == 0) {
                ...
                d.setOnDismissListener(dialogInterface -> {
                    if (d.getResult()) {
                        Hey.updateDocument(this, FirebaseFirestore.getInstance().collection(Keys.users).document(String.valueOf(My.id)), Collections.singletonMap(Keys.units, 100), doc -> {
                            Hey.showToast(this,"ADDED 100 UNITS");
                        }, errorMessage -> {
